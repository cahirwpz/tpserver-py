...
class GetExistingDesign( AuthorizedTestSession ):
	...
	def __iter__( self ):
		packet = yield self.protocol.GetDesign( self.seq, [1] ), Expect( 'Design' )
...
class GetNonExistentDesign( AuthorizedTestSession ):
	...
	def __iter__( self ):
		packet = yield self.protocol.GetDesign( self.seq, [self.WrongDesignId] ), Expect( 'Design', ('Fail', 'NoSuchThing') )
...
class GetMultipleDesigns( AuthorizedTestSession ):
	...
	def __iter__( self ):
		s, p1, p2 = yield self.protocol.GetDesign( self.seq, [1, 2] ), Expect( ('Sequence', 2, 'Design' ) )
